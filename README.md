...
This was built in a five-day sprint for a competition. It is a prototype, and I would rather
be precise about what it does than oversell it:

- **The evaluation numbers are on synthetic data.** The severity model is trained and scored
  against `synthetic_train_v1.csv`, which is generated by
  [`ml/scripts/synthesize_red_flag_dataset.py`](ml/scripts/synthesize_red_flag_dataset.py) —
  not on real patient records. A recall figure measured on data you generated yourself proves
  the pipeline runs end to end. It says nothing about clinical safety, and I make no clinical
  claim from it.
- **The hosted demo does not run the ML backend.** It runs the nine deterministic red-flag
  rules client-side (see `mockTriage()` in `frontend/lib/api.ts`) so the UI is explorable
  without standing up FastAPI, Postgres and Qdrant. The XGBoost model, the RAG retrieval layer
  and the LLM extraction step exist in this repo but are **not** deployed anywhere. To exercise
  the real pipeline you have to run the backend locally — see *Getting started*.
- **The red-flag engine itself is real** and is the part I would defend: nine pure rule
  functions, an escalate-only invariant (`final_level = max(rule_level, ml_level)`, so the ML
  model can raise a care level but never lower one), and a property test that enforces it.

Decision support only. Not a medical device, not clinically validated, not a substitute for a
clinician.
